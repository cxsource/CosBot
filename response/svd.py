...
def get_region_id(country, state):
    ...
    try:
        response = requests.get(f'https://ms-api.sivasdescalzo.com/api/regions/{country}', headers=headers, data=payload)
...
def SVDEntry(profiles, proxy, tasknumber):
    ...
    json_data = {
        'address': {
            ...
            'region_id': get_region_id(profiles['country'], profiles['region']),
            'street': [
                profiles['address'],
            ],
    ...
    json_data = {
        'participation': {
            ...
            'shipping_region_id': get_region_id(profiles['country'], profiles['region']),
            ...
            'billing_region_id': get_region_id(profiles['country'], profiles['region']),
...
def svd_main():
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for row in rows_list:
            ...
            index +=1
            # logger("Waiting 5sec before next task...", "SVD", "info", tasknumber="Sleeping")
            # time.sleep(5)
