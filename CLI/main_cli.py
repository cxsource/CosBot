...
from module.Woodwood.woodwood import woodwood_main
from ultilities.logger import qstyle
import ctypes
os.system("")
...
def menu():
	...
	setTitle(f"Cosphix Software - Welcome {getusername()}")
	...
	print(f"{colors.CBLUE} 1. Afew                                             5. SVD {colors.CGREY}(CC CURRENTLY NOT AVAILABLE){colors.RESET}")
	print(f"{colors.CBLUE} 2. BSTN {colors.RESET}")
	print(f"{colors.CBLUE} 3. Naked {colors.RESET}")
	print(f"{colors.CBLUE} 4. Woodwood {colors.RESET}")
	print(f"{colors.RED} X. EXIT\n{colors.RESET}")
    
    # print("Enter Selection: ")
	option = int(input(f"{colors.CBLUE}Enter Selection: "))
	return option
...
def CLI():
	...
	if user_choice == 0:
        ...
        "Which module do you want to start?", style=qstyle(),
        choices=[
			"Email Scraper",
			"Email Confirmer"
			]
		...
		if mode == "Email Scraper":
			scraper = questionary.select(
				"Which site do you want to scrape?", style=qstyle(),
				choices=[
					"Afew - Scraper",
					"BSTN - Scraper"
			...
			if scraper == "Afew - Scraper":
				...
				endTask = questionary.confirm(
					"Do you want to go back to the menu?", style=qstyle()
				).ask()
				if endTask == True: 
					CLI()
