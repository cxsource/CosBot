...
def AfewEntry(profile, proxy, tasknumber):
    ...
    logger(f"Submitting Your Size.. {profile['email']}", "AFEW", tasknumber=tasknumber)
    try:
        url = profile["link"]
    ...
    if response.status_code != 200: 
        ...
        logger("Submitted Raffle Information.." , "AFEW", tasknumber=tasknumber)
    ...
    if response.status_code != 200:
        ...
        logger("Submitted Shipping Information..", "AFEW", tasknumber=tasknumber)
    ...
    if response.status_code != 200:
        ...
        logger("Submitted Payment Information..", "AFEW", tasknumber=tasknumber)
    ...
    if response.status_code == 200:
        ...
        logger(f"Error while submitting the raffle entry: {response.status_code}", "AFEW", "error", tasknumber=tasknumber)
...
def afew_main1():
    ...
    if threads == 1:
        ...
        answer = questionary.confirm(
					"Do you want to restart failed tasks?"
				).ask()
